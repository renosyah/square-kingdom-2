...
func special_ability_activated():
	...
	_switch_fire_boulder(_use_special_ability)
	
func _switch_fire_boulder(v :bool):
	if not is_instance_valid(_siege_engine):
		return
	
	if not ("current_boulder" in _siege_engine):
		return
	
	_siege_engine.current_boulder = _siege_engine.fire_boulder_projectile_scene if v else _siege_engine.boulder_projectile_scene
